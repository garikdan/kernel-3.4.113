...
int mt_printk_deferred(const char *fmt, ...);
...
static DEFINE_PER_CPU(int, printk_deferred_length);
...
void printk_tick(void)
{
	if (__this_cpu_read(printk_pending)) {
		...
		if (pending & PRINTK_PENDING_SCHED) {
            ...
            __get_cpu_var(printk_deferred_length) = 0;
		}
...
static DEFINE_PER_CPU(char [PRINTK_BUF_SIZE], mt_printk_deferred_buf);
static DEFINE_PER_CPU(int, mt_printk_deferred_length);
void mt_printk_tick(void)
{
	if (__this_cpu_read(mt_printk_pending)) {
		int pending = __this_cpu_xchg(mt_printk_pending, 0);
		if (pending & PRINTK_PENDING_SCHED) {
			char *buf = __get_cpu_var(mt_printk_deferred_buf);
			printk(KERN_WARNING "[Error Loglevel Usage:start]\n");
			printk(KERN_WARNING "%s", buf);
			printk(KERN_WARNING "[Error Loglevel Usage:end]\n");
		}
	}
    __get_cpu_var(mt_printk_deferred_length) = 0;
}
#else
void mt_printk_tick(void)
{
...
int mt_printk_deferred(const char *fmt, ...)
{
    ...
    int buf_length;
	local_irq_save(flags);
	buf = __get_cpu_var(mt_printk_deferred_buf);
    buf_length = __get_cpu_var(mt_printk_deferred_length);

	va_start(args, fmt);
    if(PRINTK_BUF_SIZE >= buf_length){
	    r = vsnprintf((buf_length + buf), PRINTK_BUF_SIZE-buf_length, fmt, args);
        __get_cpu_var(mt_printk_deferred_length) += r;
    }else{
