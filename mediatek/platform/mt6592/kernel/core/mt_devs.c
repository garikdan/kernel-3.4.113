...
#endif
#if defined(FM36)
struct platform_device fm36_device = {
	.name = "fm36_driver",
	.id = -1,
};
#endif
#if defined(BQ27520)
struct platform_device bq27520_device =
{
	.name    = "bq27520",
	.id    = -1,
};
...
#if defined(BQ27520)
	retval = platform_device_register(&bq27520_device);

	if(retval != 0)
	{
		printk("platform_device_register bq27520 error:(%d)\n", retval);
	}
	else
	{
		printk("platform_device_register bq27520 done!\n");
	}
#endif
...
#if defined(AW2013)
	retval = platform_device_register(&breathlight_platform_device);
	if (retval != 0) {
		return retval;
	}
#endif
...
#endif
#if defined(FM36)
	retval = platform_device_register(&fm36_device);
	printk("register fm36 device\n");

	if(retval != 0)
	{
		printk("platform_device_register fm36 error:(%d)\n", retval);
	}
	else
	{
		printk("platform_device_register fm36 done!\n");
	}
#endif
