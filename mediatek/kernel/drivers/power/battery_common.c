...
const unsigned int cf_bat = 4320;
#else
const unsigned int cf_bat = 4170;
#endif
...
#if defined(BQ27520)
static int hw_fg_get_battery_percentage(void)
{
	unsigned int percentage = 0;
	percentage = bq27520_get_battery_percentage();
	if (percentage == 100 && BMT_status.charger_exist == KAL_TRUE)
	{
		if(BMT_status.bat_vol > cf_bat)
		{
			percentage = 100;
		}
		else
...
{ 
	...
	if(bat_meter_timeout == KAL_TRUE || bat_spm_timeout == TRUE)
	{
		...
		if(Bq27520_Exist)
		{
			SOC = hw_fg_get_battery_percentage();
		}
		else
		{
	...
	if(Bq27520_Exist)
	{
		BMT_status.ICharging =bq27520_get_battery_current();
		BMT_status.bat_vol = bq27520_get_battery_vol();
		BMT_status.temperature = bq27520_get_battery_temprature();
	}
	else
	{
...
void do_chrdet_int_task(void)
{
    if(g_bat_init_flag == KAL_TRUE)
    {
		...
		if(Bq27520_Exist)
		{
			if (BMT_status.UI_SOC == 100 && BMT_status.bat_vol > cf_bat && BMT_status.ICharging < 90 && BMT_status.charger_exist == KAL_TRUE)
			{
		 ...
		 if (BMT_status.bat_vol > 0)
		{
		 ...
		 if(g_battery_soc_ready == KAL_FALSE)
		 {
			 ...
			 if(Bq27520_Exist)
			 {
				 BMT_status.SOC = hw_fg_get_battery_percentage();
			 }
			 else
			 {
...
static int battery_resume(struct device *device)
{
	...
	if(g_call_state == CALL_ACTIVE && (bat_time_after_sleep.tv_sec - g_bat_time_before_sleep.tv_sec >= TALKING_SYNC_TIME))	// phone call last than x min
	{
		...
		if(Bq27520_Exist)
		{
			BMT_status.UI_SOC = hw_fg_get_battery_percentage();
		}
		else
		{
